...
final class Delete extends Component
{
    ...
    {
        HeadManager::$title = I18n::get("pages.deleteEdition");
        $conn = Connection::get();
        try
        {
    ...
    {
        return isset($this->edition) ? component(DefaultPageFrame::class, children:
        [
            tag('h1', children: text(I18n::get('pages.deleteEdition'))),

            tag('delete-entity-form',
                delete_function_route_url: URLGenerator::generateFunctionUrl("/admin/panel/magazines/{$this->edition->magazine->id->unwrapOr(0)}/editions/{$this->edition->id->unwrapOr(0)}"),
                go_back_to_url: "/admin/panel/magazines/{$this->edition->magazine->id->unwrapOr(0)}",
                langJson: Data::hscq(I18n::getFormsTranslationsAsJson()), 
                children:
                [
                    component(Label::class, labelBold: true, label: I18n::get('pages.magazine'), children: text($this->edition->magazine->name->unwrapOr(''))),
