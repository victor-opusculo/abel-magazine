...
final class Functions extends BaseFunctionsClass
{
    ...
    public function setEditorialTeamPageId(array $data) : array
    {
        $conn = Connection::get();
        try
        {
            [ 'page_id' => $newId, 'page_id_en' => $newIdEnglish, 'remove' => $remove ] = $data;
            $sett = new EditorialTeamPageId();
            $settEn = new EditorialTeamPageIdEnglish();
            ...
            if ($result['affectedRows'] > 0)
            {
                LogEngine::writeLog("Páginas de equipe editorial definidas! ID: {$newId}");
                return [ 'success' => I18n::get('functions.settingChangeSuccess') ];
            }
            else
            {
