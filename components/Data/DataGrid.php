...
class DataGrid extends Component
{
    ...
    protected function markup(): Component|array|null
    {
        ...
        if (!isset($this->dataRows[0]))
            return text(I18n::get('forms.noDataAvailable'));
            ...
            [
                ...
                [
                    ...array_map( function($col) use (&$colCount)
                    {
                        ...
                        {
                            if (isset($this->detailsButtonURL) && !isset($this->columnNameAsDetailsButton)) $comps[] = tag('th', class: 'w-5', children: [ text(I18n::get('forms.details')) ]);
                            if (isset($this->editButtonURL)) $comps[] = tag('th', class: 'w-5', children: [ text(I18n::get('forms.edit')) ]);
                            if (isset($this->deleteButtonURL)) $comps[] = tag('th', class: 'w-5', children: [ text(I18n::get('forms.delete')) ]);
                            if (isset($this->selectButtonOnClick)) $comps[] = tag('th', class: 'w-5', children: [ text(I18n::get('forms.select')) ]);
                    ...
                    [
                        ...array_map( function($column, $value) use (&$colCount2, &$row)
                        {
                            ...
                            if ($colCount2 === count($row))
                            {
                                ...
                                [ 
                                    tag('a', class: 'link text-lg', href: str_replace('{param}', $row[$this->rudButtonsFunctionParamName], $this->detailsButtonURL), children: [ text(I18n::get('forms.details')) ])
                                ]);
                                if (isset($this->editButtonURL)) $comps[] = tag('td', ...[ 'data-th' => 'Editar' ], class: 'w-5', children: 
                                [ 
                                    tag('a', class: 'link text-lg', href: str_replace('{param}', $row[$this->rudButtonsFunctionParamName], $this->editButtonURL), children: [ text(I18n::get('forms.edit')) ])
                                ]);
                                if (isset($this->deleteButtonURL)) $comps[] = tag('td', ...[ 'data-th' => 'Excluir' ], class: 'w-5', children: 
                                [ 
                                    tag('a', class: 'link text-lg', href: str_replace('{param}', $row[$this->rudButtonsFunctionParamName], $this->deleteButtonURL), children: [ text(I18n::get('forms.delete')) ])
                                ]);
                                if (isset($this->selectButtonOnClick)) $comps[] = tag('td', ...[ 'data-th' => 'Selecionar' ], class: 'w-5', children: 
                                [
                                    tag('a', class: 'link text-lg', href: str_replace('{param}', $row[$this->rudButtonsFunctionParamName], $this->selectButtonOnClick), children: [ text(I18n::get('forms.select')) ])
                                ]);
